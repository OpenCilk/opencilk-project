; RUN: opt < %s -passes=simplifycfg -S | FileCheck %s --check-prefix=SC
; RUN: opt < %s -passes=simplifycfg,task-simplify -post-cleanup-cfg=false -S | FileCheck %s --check-prefix=TS
