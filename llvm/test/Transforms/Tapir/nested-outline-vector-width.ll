; RUN: opt < %s -loop-spawning-ti -simplifycfg -function-attrs -tapir2target -use-opencilk-runtime-bc=false -debug-abi-calls -S | FileCheck %s
; RUN: opt < %s -loop-spawning-ti -simplifycfg -function-attrs -tapir2target -always-inline -use-opencilk-runtime-bc=false -debug-abi-calls -S | FileCheck %s
; RUN: opt < %s -passes="loop-spawning,function(simplify-cfg),cgscc(function-attrs),tapir2target" -use-opencilk-runtime-bc=false -debug-abi-calls -S | FileCheck %s
; RUN: opt < %s -passes="loop-spawning,function(simplify-cfg),cgscc(function-attrs),tapir2target,always-inline" -use-opencilk-runtime-bc=false -debug-abi-calls -S | FileCheck %s
