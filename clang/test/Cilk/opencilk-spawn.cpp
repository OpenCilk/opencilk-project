...
// CHECK: [[LABEL1]]
// CHECK: call void @_ZN3BarC1Ev(ptr {{.*}}dereferenceable(16) %[[REFTMP]])
// CHECK: %[[CALL:.+]] = invoke {{.*}}i32 @_ZNK3Bar11getValSpawnEi(ptr {{.*}}dereferenceable(16) %[[REFTMP]], i32 noundef 0)
// CHECK: call void @_ZN3BarD1Ev(ptr {{.*}}dereferenceable(16) %[[REFTMP]])
// CHECK: call void @_ZN3BarC1Ev(ptr {{.*}}dereferenceable(16) %[[REFTMP2]])
// CHECK: %[[CALL:.+]] = invoke {{.*}}i32 @_Z3fooRK3Bar(ptr {{.+}}%[[REFTMP2]])
// CHECK: call void @_ZN3BarD1Ev(ptr {{.*}}dereferenceable(16) %[[REFTMP2]])
// CHECK-NEXT: br label %[[LABEL1]]
// CHECK: [[CONTINUE]]
// CHECK: ret void

// CHECK-LABEL: define linkonce_odr {{(dso_local )?}}{{.*}}i32 @_ZNK3Bar11getValSpawnEi(ptr
// CHECK: detach within %[[SYNCREG:.+]], label %[[DETACHED:.+]], label %[[CONTINUE:.+]]
// CHECK: [[DETACHED]]
// CHECK: %[[CALL:.+]] = call noundef i32 @_Z12return_stuffi(i32
// CHECK-NEXT: store i32 %[[CALL]]
