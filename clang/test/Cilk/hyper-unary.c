...
void function1()
{
  ...
  // should be call i8* @llvm.hyper.read(i8* bitcast (i32* @x to i8*))
  // CHECK: load i32
  // CHECK: %[[Y1:.+]] = bitcast i32* %[[Y]] to i8*
 // CHECK: call i8* @llvm.hyper
  // should be call i8* @llvm.hyper.read(i8* %[[Y1]])
  // CHECK: load i32
  (void)x; (void)y;
}
...
void function2()
{
  ...
  // CHECK: call i8* @llvm.hyper.
  // should be call i8* @llvm.hyper.read(i8* %[[Y2]])
  // CHECK: load i32
  (void)!x; (void)!y;
}
...
void function3()
{
  ...
  // CHECK: call i8* @llvm.hyper.
  // should be call i8* @llvm.hyper.read(i8* %[[Y3]])
  // CHECK: load i32
  (void)-x; (void)-y;
  // CHECK: call i8* @llvm.hyper.
  // should be call i8* @llvm.hyper.read(i8* bitcast (i32* @x to i8*))
  ...
  // CHECK: call i8* @llvm.hyper.
  // should be call i8* @llvm.hyper.read(i8* %[[Y4]])
  // CHECK: load i32
  (void)~x; (void)~y;
  // CHECK: %[[XP:.+]] = load i32*, i32** @xp
  // CHECK: %[[XP1:.+]] = bitcast i32* %[[XP]] to i8*
  // CHECK: call i8* @llvm.hyper.
  // should be call i8* @llvm.hyper.read(i8* %[[XP1]])
  // CHECK: load i32
  (void)*xp;
}
